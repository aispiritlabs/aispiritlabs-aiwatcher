...
final readonly class Dataset
{
    ...
    public function __construct(
        ...
        /**
         * Where the rows live on disk, when they are not an API route.
         *
         * A name under the service's corpus root (`AIWATCHER_CORPUS_DIR`),
         * holding `<corpus>.csv/part-*.csv` and `<corpus>.parquet/part-*.parquet`.
         * A query names the dataset and never a path: the catalog decides what
         * may be read, which is why `Loader`, `Extractor` and `Filesystem` stay
         * outside the language. For such a dataset `path`, `rowsPath` and
         * `cursorParam` describe nothing and are never read.
         */
