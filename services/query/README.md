...
| [`contract/`](contract) | `aiwatcher_query`: what every Python engine serves — the seven `/query` routes, the catalog all three engines load ([`catalog.json`](contract/catalog.json)), the API paged into Arrow, the fork server and the child a query runs in, `strict` admission, and the conformance suite | — | `just query-contract-check` |
