...
async fn start_scoring_run(
    ...
    // What a project's run may be made of is not decided here:
    // `aiwatcher_execution::start` refuses a project's plan whose steps have
    // no project performer, naming each one and why
    // (`RuntimeKind::outside_a_project`). A second list in this route would be
    // a second answer to one question, and the one that drifted would be the
    // one a new runtime was added to.
