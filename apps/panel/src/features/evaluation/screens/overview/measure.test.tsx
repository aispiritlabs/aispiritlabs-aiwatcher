...
it('declares a candidate and its baseline, each generating its answers the same way', async () => {
  const baselineResult = {
    ...published(),
    receipt: { ...published().receipt, evaluation_id: 'baseline-0' },
    manifest: {
      ...published().manifest,
      variant: { ...published().manifest.variant, experiment_id: 'old-prompt' },
    },
  };
  const server = serve([
    { method: 'GET', path: '/auth/config', answer: { status: 200, body: { enabled: false } } },
    ...
    },
    {
      method: 'GET',
      path: '/evaluation-scorecards/answer-quality',
      answer: { status: 200, body: card({ kind: 'exact_match' }) },
    },
    {
      method: 'GET',
      path: '/evaluation-results',
      answer: {
        status: 200,
        body: { evaluations: [published(), baselineResult], next_cursor: null },
      },
    },
    {
      method: 'POST',
  ...
  await choose();
  await userEvent.type(screen.getByLabelText('Evaluation ID'), 'candidate-1');
  await userEvent.click(screen.getByLabelText('Generated by a worker'));
  await userEvent.type(screen.getByLabelText('Generation task'), 'support-bot.answer@3');
  await userEvent.type(screen.getByLabelText('Generation queue'), 'evaluation');
  await userEvent.selectOptions(screen.getByLabelText('Baseline'), 'baseline-0');
  expect(screen.getByText(/publishing as candidate-1-baseline/)).toBeTruthy();
  await userEvent.click(screen.getByRole('button', { name: 'Declare' }));

  await waitFor(() => expect(server.countOf('POST', '/evaluation-runs')).toBe(2));
  expect(onDeclared).toHaveBeenCalledWith(DECLARATION, DECLARATION);
  const [candidate, baseline] = server.calls
    .filter((call) => call.method === 'POST' && call.url.endsWith('/evaluation-runs'))
    .map((call) => call.body) as [Record<string, any>, Record<string, any>];
  expect(candidate.answers).toEqual({
    generated_by: { task: 'support-bot.answer@3', queue: 'evaluation' },
  });
  expect(candidate.variant.experiment_id).toBe('baseline');
  expect(baseline.evaluation_id).toBe('candidate-1-baseline');
  expect(baseline.variant.experiment_id).toBe('old-prompt');
  // Measured the same way: everything but the variant and the ID is one declaration.
  expect(baseline.answers).toEqual(candidate.answers);
  expect(baseline.cohort).toEqual(candidate.cohort);
  expect(baseline.scorecard).toEqual(candidate.scorecard);
  expect(server.countOf('PUT', '/evaluation-recordings/answers.json')).toBe(0);
});
