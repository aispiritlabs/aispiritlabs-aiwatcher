...
import { getNotebook, getNotebookRevision, NotebookError, saveNotebook } from '@/shared/lib/ml-pipeline';
import {
  exportBundle,
  importBundle,
  parseBundle,
...
import { runPipeline } from '@/features/data-curation/lib/pipeline';
import { runNotebook } from '@/shared/lib/ml-pipeline';
import { simulateQuery } from '@/shared/lib/query';

vi.mock('@/shared/lib/ml-pipeline', async (original) => ({
  ...(await original<typeof import('@/shared/lib/ml-pipeline')>()),
  getNotebook: vi.fn(),
  getNotebookRevision: vi.fn(),
  saveNotebook: vi.fn(),
  runNotebook: vi.fn(),
}));
vi.mock('@/shared/lib/query', async (original) => ({
  ...(await original<typeof import('@/shared/lib/flow')>()),
  simulateQuery: vi.fn(),
}));
