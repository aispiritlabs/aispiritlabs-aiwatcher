...
export const NEW_TRANSFORM = '->limit(100)';

/** How a transform block's text is written, for the block inspector. */
export const TRANSFORM_HELP =
  'The tail of the pipeline, one step per line. No data_frame(), no read() and no write() — the chain contributes those, and every Flow block before the first notebook runs as one query.';

/** The recipe editor's cheat sheet: one line of Flow per thing a curation does. */
